...
test(writeAndReadByte)
{
  ...
  eeprom.write(pos, in);
  assertEqual(eeprom.getLastError(), ERR_OK);
...
test(putDoubleAndGetBack)
{
  ...
  eeprom.put(pos, pi);
  assertEqual(eprom.getLastError(), ERR_OK);
  double pi_in;
...
test(badAddressErrorBusWorksAfter)
{
  ...
  assertEqual(badEeprom.getLastError(), ERR_OK);
  badEeprom.write(pos, in);
  assertEqual(badEeprom.getLastError(), ERR_NACK);
