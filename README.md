# AT24C Two-Wire (I2C) Serial EEPROM Library
...
```C++
int foo = 42;
eeprom.put(0, foo);      // Write the integer value 42 to address 0
int foo_in;
eeprom.get(0, foo_in);   // Read the integer value at address 0 into variable foo_in
```
...
eeprom.put(0, point);     // Write the struct point to address 0
Coordinate point_in;
eeprom.get(0, point_in);  // Read the values of the struct point_in from address 0
```
...
eeprom.writeBuffer(0, out, 15);
uint8_t in[15];
eeprom.readBuffer(0, in, 15);
```
## Reading raw bytes
The library can also read write and update single bytes:
```C++
eeprom.write(0, 77);             // Writes the value 77 to byte at address 0
eeprom.update(0, 77);            // In this case `update` does nothing, since it only writes if the value differs from the current
uint8_t value = eeprom.read(0);  // Reads the value of the byte at address 0
```
...
AT24C256 eeprom0(AT24C_ADDRESS_0);
AT24C256 eeprom1(AT24C_ADDRESS_1);
AT24C02 eeprom2(AT24C_ADDRESS_2);

void setup() {
  Wire.begin();
 
  uint8_t byte = eeprom0.read(0);
}
```
## Error handling
Since this is an external memory chip that is connected through the I2C bus, there is always a risk that the communication fails due to physical errors in the setup. You can always check if the latest operation succeeded via the `getLastError()` method:
```C++
uint8_t data = eeprom.read(0);
if (eeprom.getLastError() != 0) {
...
Some Arduino boards have multiple I2C buses. The library allows you to specify which TwoWire bus to use for each chip object:
```C++
#include <at24c256.h>

AT24C256 eeprom0(AT24C_ADDRESS_0, Wire);
```
...
AT24C256 eeeprom0(AT24C_ADDRESS_0, Wire, 20);
```
If your application is really time critical and you only write single bytes, you can also set the write cycle time to 0 to avoid the internal wait. In that case you have to keep track on that your application does not try to access the chip too fast after a write operation.
...
AT24C04 eeprom0(AT24C04_ADDRESS_1);
AT24C08 eeprom1(AT24C08_ADDRESS_1);
AT24C16 eeprom2();
```
