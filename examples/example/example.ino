...
// Create a eeprom object configured at address 0
...
AT24C256 badEeprom(AT24C_ADDRESS_2);
...
setup()
{
  ...
  int lastError = eeprom.getLastError();
  ...
  badEeprom.read(0);
  // Get the last error code, it should not be zero, but 2 which means that there were no response from the eprom
  int lastError3 = badEeprom.getLastError();
